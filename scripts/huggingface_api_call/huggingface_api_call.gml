/// huggingface_api_call - Fixed version
function huggingface_api_call(prompt, callback) {
    ...
    if (global.offline_mode) {
        ...
        // Use time_source to simulate API delay
        var timer = time_source_create(time_source_global, 1, time_source_units_seconds, 
            function(_callback, _response) {
                _callback(_response);
            }, 
            [[callback, fallback_response]], 1);
    ...
    var timeout_timer = time_source_create(time_source_global, 10, time_source_units_seconds, 
        function(_request_id, _prompt, _callback) {
            if (ds_map_exists(global.api_callbacks, string(_request_id))) {
                // Extract NPC ID from conversation history
                var npc_id = "";
                for (var key = ds_map_find_first(global.npc_prompts); key != undefined; key = ds_map_find_next(global.npc_prompts, key)) {
                    if (string_pos(global.npc_prompts[? key], _prompt) > 0) {
                        npc_id = key;
                        break;
                    }
                }
                ...
                // Determine if this is initial message or follow-up
                var is_initial = string_pos("Player: Hi, can you help me?", _prompt) > 0;

                // Get fallback response
                var fallback_response = handle_api_failure(npc_id, is_initial);

                // Execute callback with fallback response
                _callback(fallback_response);
				show_debug_message("API response callback, response=" + response);
				show_debug_message("Queue empty? " + string( ds_queue_empty( message_queue ) ));
